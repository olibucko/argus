...
class CameraCapture:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if not self.cap.isOpened():
                logger.error(f"Failed to open video source: {self.config.camera_id}")
                return False
            ...
            self.connected = True
            logger.info(f"Camera '{self.config.camera_name}' connected successfully")
            return True
            
        except Exception as e:
            logger.error(f"Failed to connect camera '{self.config.camera_name}': {e}")
            if self.cap:
                self.cap.release()
            return False
    ...
    def read_frame(self) -> Tuple[bool, Optional[np.ndarray]]:
        ...
        try:
            ...
            logger.error(f"Error reading from camera '{self.config.camera_name}': {e}")
            self.connected = False
            return False, None
    ...
    def start_all(self) -> bool:
        ...
        for camera_name, camera in self.cameras.items():
            if camera.connect():
                ...
                logger.warning(f"Failed to start camera: {camera_name}")
                
        logger.info(f"Started {success_count}/{len(self.cameras)} cameras")
    ...
    def _camera_loop(self, camera_name: str, camera: CameraCapture):
        ...
        while self.running:
            if not camera.connected:
                ...
                logger.info(f"Attempting to reconnect camera: {camera_name}")
                if camera.connect():
                    consecutive_failures = 0
                    ...
                    if consecutive_failures >= max_failures:
                        logger.error(f"Camera {camera_name} failed too many times. Stopping.")
                        break
                ...
                if self.frame_broker:
                    ...
                    if not self.frame_broker.submit_frame(camera_name, frame, timestamp):
                        ...
                        if camera_name in camera_status:
                            ...
                            logger.warning(f"Frame dropped from {camera_name} - drop_rate: {status['drop_rate']*100:.1f}%, "
                                  f"consecutive_drops: {status['consecutive_drops']}, current_fps: {status['current_fps']:.1f}")
                        else:
                            logger.error(f"Frame dropped from {camera_name} - camera not registered with frame broker!")

            else:
                consecutive_failures += 1
                logger.warning(f"Frame read failed for {camera_name} (failures: {consecutive_failures})")

                if consecutive_failures >= max_failures:
                    logger.error(f"Too many failures for {camera_name}. Will attempt reconnection.")
                    camera.disconnect()
                    time.sleep(camera.config.retry_interval)
