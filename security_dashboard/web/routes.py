...
def video_feed(row: int, col: int):
    ...
    def generate_frames(viewport_id: tuple):
        ...
        logger.debug(f"Video feed requested for viewport {viewport_id}")
        logger.debug(f"Available encoded frames: {list(security_system.encoded_frames.keys())}")
        ...
        while True:
            ...
            if encoded_frame is not None:
                if not first_frame_sent:
                    logger.debug(f"First frame sent for viewport {viewport_id}")
                try:
                    yield (b'--frame\r\nContent-Type: image/jpeg\r\n\r\n' + encoded_frame + b'\r\n')
                except (BrokenPipeError, ConnectionResetError):
                    # Client disconnected, exit the generator cleanly
                    logger.debug(f"Client disconnected for viewport {viewport_id}")
                    return
                ...
                logger.warning(f"Timeout for viewport {viewport_id} - no frames after {max_wait}s")
