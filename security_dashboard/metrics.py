...
class SystemMetrics:
    ...
    def get_metric_summary(self, name: str, duration_seconds: float = 300) -> Dict[str, Any]:
        """
        Get a comprehensive summary for a specific metric over a time duration.

        Args:
            name (str): The name of the metric to summarize.
            duration_seconds (float): The time window in seconds to calculate the average over.

        Returns:
            Dict[str, Any]: A dictionary containing the latest value, timestamp,
                            average, sample count, and other summary details.
        """
        with self.lock:
            if name not in self.collectors:
            ...
            if not collector.values:
                return {"name": name, "latest_value": None, "average": None, "sample_count": 0}

            latest = collector.values[-1]
